...
def create_sentence_plot(sentence="a year from now you will wish you started today", bg_color='black', txt_color='white', line_color='white'):
    ...
    plt.savefig("outputv5.jpg", facecolor=fig.get_facecolor(), edgecolor='none')
...
def get_sentence():
    def submit():
        ...
        if sentence:  # Check if the user input is not empty
            create_sentence_plot(sentence, bg_color, txt_color, line_color)
        else:
            create_sentence_plot(bg_color=bg_color, txt_color=txt_color, line_color=line_color)  # Use the default sentence if the user input is empty
